...
/*Funcion para listar sus publicaciones de la ayuda social*/ 

const listHelpSocialsToken = async (req, res) => {
  try {
    const name = req.user.name;
    const idUser = req.user._id;

    const helpSocial = await HelpSocial.find()

    if (!idUser) return res.status(500).json({ message: "No se ha encontrado el usuario" });

    return res.status(200).json({ message: `Publicaciones del usuario ${name}`, helpSocial, idUser });

} catch (error) {
    console.log(error);
    return res.status(500).json({ message: "Error al obtener las publicaciones del foro" });
}
};

/*Funcion para listar sus publicaciones del foro*/

const listForumToken = async (req, res) =>{
    try {
        const name = req.user.name;
        const idUser = req.user._id;
        ...
        return res.status(200).json({ message: `Publicaciones del usuario ${name}`, forums, idUser });
